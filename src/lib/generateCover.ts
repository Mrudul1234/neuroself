...
export const generateNeuroShelfCover = async (
  ...
  const stylePrompts = {
    paper: `vintage academic book cover, cream parchment texture background, 
      ...
      border, botanical scientific engraving style, title: "${cleanedTitle}", 
    ...
    article: `vintage journal magazine cover, cream ivory background, 
      ...
      hand-drawn scientific illustration style, title: "${cleanedTitle}", 
      scholarly yet beautiful, no cluttered elements, 
      not photorealistic, not modern digital, not neon, not white background`,

    video: `modern educational book cover, cream background with 
      ...
      clean minimal academic poster style, title: "${cleanedTitle}", 
  ...
  const url = `https://image.pollinations.ai/prompt/${prompt}?width=400&height=560&seed=${seed}&model=flux&nologo=true`;

  console.log("[NeuroShelf Cover] Constructed URL:", { title, cleanedTitle, type, url });
  return url;
};
